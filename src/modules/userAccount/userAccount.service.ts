...
export class UserAccountService {
  ...
  public async register(
    ...
    const userProfile =
      await this.userProfileRepository.createProfileByUserAccountCode(
        userAccount.code,
        transaction
      );

    const userRole = await this.userRoleRepository.findOneByUser(
      userAccount.code
    );
    ...
    await mailService.send(
      ...
      "email-verification",
      { username: username ?? email, verificationUrl }
    );
  ...
  public async confirmEmail(
    ...
    await this.userTokenRepository.softDelete(
      { value: token },
      { transaction }
    );
  }
