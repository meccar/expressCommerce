export const messages = {
  ...
  cluster: {
    ...
    forkingWorkers: (numCPUs: number): string => `Forking ${numCPUs} workers...`,
    workerDied: (pid: number | undefined, code: number, signal: string): string =>
      `Worker ${pid} died (exit code: ${code}, signal: ${signal})`,
    newWorkerForked: (pid: number | undefined): string =>
      `Forked a new worker ${pid} to replace the dead worker`,
    workerStarted: (pid: number, port: string): string => `Worker ${pid} started on port ${port}`,
  },
  ...
  service: {
    configured: (service: string): string => `${service} has been successfully configured.`,
    notConfigured: (service: string): string => `${service} has not been configured.`,
  },
} as const;
