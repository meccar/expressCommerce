...
export class SignInManager<TUser extends object> {
  ...
  public async passwordSignInAsync(
    ...
    const user = await this.userAccountRepository.findByEmailOrUsername(
      email,
      username
    );
    ...
    if (!user.isActive) return { succeeded: false, isNotAllowed: true };

    if (user.lockoutEnd && new Date(user.lockoutEnd) > new Date())
      return { succeeded: false, isLockedOut: true };

    const passwordValid = await this.userAccountRepository.verifyPasswordAsync(
      user,
      password
    );

    if (passwordValid !== PasswordVerificationResult.Success) {
      if (lockoutOnFailure) await this.incrementAccessFailedCount(user);

      return { succeeded: false };
    }

    if (user.accessFailedCount > 0) await this.resetAccessFailedCount(user);
  ...
  public async externalLoginSignInAsync(
    ...
    const user = await this.authService.validateExternalProviderToken(
      loginProvider,
      providerKey
    );
  ...
  public async twoFactorSignInAsync(
    ...
    if (!user || !user.twoFactorEnabled) return { succeeded: false };

    const isValidCode = await this.verifyTwoFactorCode(user, twoFactorCode);

    if (!isValidCode) return { succeeded: false };
  ...
  public async refreshTokenAsync(token: string): Promise<SignInResult> {
    ...
    if (!user || !user.isActive) return { succeeded: false };
